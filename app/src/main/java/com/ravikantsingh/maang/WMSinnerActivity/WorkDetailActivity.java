...
public class WorkDetailActivity extends AppCompatActivity {
    ...
    void init() {
        recyclerView = findViewById(R.id.work_detail_work_recycler);
        adapter = new RecommendedWorkAdapter(this, list);
        mLayoutManager = new LinearLayoutManager(this);
        recyclerView.setLayoutManager(mLayoutManager);
        recyclerView.setAdapter(adapter);
        reference = FirebaseDatabase.getInstance().getReference().child(StringVariables.WMS_REPORT).child("hajipur").child(StringVariables.WMS_DETAIL);
    }
