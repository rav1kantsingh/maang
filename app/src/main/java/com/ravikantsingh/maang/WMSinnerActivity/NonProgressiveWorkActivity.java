...
public class NonProgressiveWorkActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_non_progressive_work);
        list = new ArrayList<>();
        init();
        loadAndUpdateUI();
    }
    ...
    void init() {
        recyclerView = findViewById(R.id.progressive_work_recycler);
        adapter = new RecommendedWorkAdapter(this, list);
        mLayoutManager = new LinearLayoutManager(this);
        recyclerView.setLayoutManager(mLayoutManager);
        recyclerView.setAdapter(adapter);
        list = new ArrayList<>();
        reference = FirebaseDatabase.getInstance().getReference().child(StringVariables.WMS_REPORT).child("hajipur").child(StringVariables.WMS_NON_PROGRESSIVE);
    }
